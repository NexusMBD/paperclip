...
export function routineRoutes(
  ...
  router.get("/companies/:companyId/routines", async (req, res) => {
    ...
    res.json(result.map((r) => ({ ...r, defaultLabelIds: r.executionLabelIds ?? [], labelIds: r.executionLabelIds ?? [] })));
  });
  ...
  router.post("/companies/:companyId/routines", validate(createRoutineSchema), async (req, res) => {
    ...
    res.status(201).json({ ...created, defaultLabelIds: created.executionLabelIds ?? [], labelIds: created.executionLabelIds ?? [] });
  });
  ...
  router.get("/routines/:id", async (req, res) => {
    ...
    res.json({ ...detail, defaultLabelIds: detail.executionLabelIds ?? [], labelIds: detail.executionLabelIds ?? [] });
  });
  ...
  router.patch("/routines/:id", validate(updateRoutineSchema), async (req, res) => {
    ...
    res.json(updated ? { ...updated, defaultLabelIds: updated.executionLabelIds ?? [], labelIds: updated.executionLabelIds ?? [] } : null);
  });
